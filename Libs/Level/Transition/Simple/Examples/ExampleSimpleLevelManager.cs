...
namespace MMGame.Level.Example
{
    ...
    public class ExampleSimpleLevelManager : MonoBehaviour
    {
        [SerializeField]
        private string TargetLevelName;

        [SerializeField]
        private bool ManuallyFadeIn;
        ...
        private void OnLoadLevelCompleted(ALevelMap map, LoadMode mode)
        {
            ...
            if (map.SceneName == TargetLevelName && ManuallyFadeIn)
            {
                LevelTransition.ManuallyFadeIn = true;
                Invoke("DoManuallyFadeIn", 4);
            }
        }

        private void DoManuallyFadeIn()
        {
